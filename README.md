# 制御システム部２G　2024年度インターン（IoT編）

## 本カリキュラムの目的

本インターンは授業・課題ではないので、なるべく多くの経験をしてもらいたいと考えています。
インターネットでの検索や、AIの活用は自分の判断で自由におこなってかまいません。
...
本実習のサンプルプログラムは、以下のデバイスを利用することを想定して作成してあります。
全て通販などで手に入りやすい部品です。

- 開発ボード ESP32 ESP-WROOM-32 WiFi Bluetooth (USB Type-C)
- IRヒューマンセンサー AM312
- 温度・湿度センサー AHT10 (I2C対応)
- アナログ振動センサー （電圧センサー）
- ニューブレッドボード SAD-101（ESP32-Devの幅が独特なので6穴を使っています、通常の５穴のボードであれば２枚連結してください）
- LED（3.3Vで駆動する任意のLED１個）
- 抵抗器（3.3V電圧に応じたLEDに対する抵抗1個 200Ω程度）
- 抵抗器（5Vを3.3Vに降圧させるため、1KΩ/2KΩの抵抗各1個）
- ジャンパー線数本

### 開発環境のセットアップとESP32へのファームウエアの書き込み

次のページに環境構築の手順書を作成したので、開発環境を構築しましょう。Thonnyを使ってESP32にMicroPythonに対応したファームウェアを書き込みます。

[セットアップ手順](esp32_doc/setup.md)

## 実習: ESP32を使って実際のIoTセンサーデバイスを作成しよう

本実習では、MicroPythonを使ってプログラミングを行います。Python未経験でも、順に練習問題を解くことでIoTデバイスを作成できるようにカリキュラムを作成しました。

以下にThonnyによるMicroPythonでの開発の手順をまとめましたので、参照しながら実習を進めましょう。

[開発手順](esp32_doc/development.md)

以下に練習問題を用意しました、09で簡単なIoTデバイスが完成します、インターン期間中に進むと良いですね。
このページはしばらく残す予定ですので、振り返りやレポート作成に活用してください。

各練習問題には、以下の項目が含まれています。
"実装内容"には問題の仕様が書かれおり、それまでの練習問題と"以下を実行して結果を確認してみましょう"を組み合わせれば、解ける内容となっています。
また、各練習問題には回答例を数個用意しておきました、practiceXX-YY.pyという名前で保存しています。
YYの数字が大きい方がより実務に近い良いプログラムですので、問題を解き終わったら読んでみてください。
...
> [!Warning]
> 問題を解くことを急ぐ必要はありません、"理解"しながら自分のペースで進めてください。
> [こちら](error_handle/ReadMe.md)にエラーハンドリングをまとめておきました。
> エラーが発生したら一読してみましょう。

### IoTデバイスをステップを踏んで実装

順に技術を身に着けて、センサーデータをサーバにアップロードするIoTデバイスを作ろう。
...
- [04.オンボードの温度センサーを読み込んでみよう](practice04/ReadMe.md)
  - 数値計算をする
  - 関数を作成する
- [エラーハンドリング](error_handle/ReadMe.md)
  - エラーを発生させてみる
  - エラーハンドリングに慣れる
